const { getModeDefinition, isStopMode: isStopModePolicy } = require("./mode-policy");

function registerUiModule(app, env = {}) {
  ...
  function isCompactViewport() {
    if (typeof windowRef?.matchMedia === "function") {
      try {
        if (windowRef.matchMedia("(max-width: 679px)").matches) {
          return true;
        }
      } catch {
  ...
  function render(data) {
    ...
    for (let i = 0; i < listDepartures.length; i++) {
      ...
      const letterBadge = documentRef.createElement("div");
      ...
      const train = documentRef.createElement("div");
      train.className = "train departure-main";

      const destination = documentRef.createElement("div");
      ...
      const clockTime = documentRef.createElement("span");
